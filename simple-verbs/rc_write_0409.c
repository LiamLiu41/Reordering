...
// 打印使用方法
void print_usage(const char *program_name) {
    printf("Usage: %s [-s|-r] [-d device_name] [-i ib_port] [-a server_address] [-p port] [-n num_writes] [-g gid_idx] [-S src_ip]\n", program_name);
    printf("Options:\n");
    ...
    printf("  -g, --gid-idx       Local GID index for RoCE routing (default: 3)\n");
...
int main(int argc, char *argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, "srd:i:a:p:n:g:S:h", long_options, &option_index)) != -1) {
        switch (opt) {
            case 's':
                is_sender = 1;
                break;
    ...
    // 打印所有参数
    printf("dev_name: %s, ib_port: %d, server_addr: %s, port: %d, num_writes: %d, gid_idx: %d, src_ip: %s\n",
