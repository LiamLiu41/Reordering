...
int wait_for_write_with_imm_completion(struct rdma_resources *res);
...
int perform_rdma_write(struct rdma_resources *res, int num_writes) {
    ...
    for (int i = 0; i < num_writes; i++) {
        ...
        struct ibv_sge sge = {
            .addr = (uintptr_t)(res->buffer + offset),
            .length = length,
            .lkey = res->mr->lkey
        };
        
        struct ibv_send_wr wr = {
            .wr_id = i + 1,  // 使用不同的wr_id来跟踪请求
            .sg_list = &sge,
            .num_sge = 1,
            .opcode = IBV_WR_RDMA_WRITE,
            .send_flags = IBV_SEND_SIGNALED,
            .wr.rdma = {
                .remote_addr = res->remote_info.addr + offset,
                .rkey = res->remote_info.rkey
            }
        };
        
        printf("Sending RDMA Write #%d to remote address: %lx, rkey: %u, size: %lu bytes\n", 
    ...
    // 立即发送0-byte的RDMA_WRITE_WITH_IMM，不等待前面的Write完成
    printf("Sending 0-byte RDMA_WRITE_WITH_IMM...\n");
    
    struct ibv_sge sge_imm = {
    ...
    struct ibv_send_wr wr_imm = {
        .wr_id = 9999,  // 特殊的wr_id标识这是write_with_imm
        .sg_list = &sge_imm,
        .num_sge = 1,
        .opcode = IBV_WR_RDMA_WRITE_WITH_IMM,
        .send_flags = IBV_SEND_SIGNALED,
        .imm_data = htonl(0x12345678),  // immediate data
        .wr.rdma = {
            .remote_addr = res->remote_info.addr,
            .rkey = res->remote_info.rkey
        }
    };
    
    struct ibv_send_wr *bad_wr;
    int ret = ibv_post_send(res->qp, &wr_imm, &bad_wr);
    if (ret) {
        fprintf(stderr, "Failed to post RDMA_WRITE_WITH_IMM: %s\n", strerror(ret));
        return 1;
    }
    
    printf("RDMA_WRITE_WITH_IMM posted with immediate data: 0x%x\n", 0x12345678);

    // 现在等待所有操作完成 (num_writes个RDMA Write + 1个RDMA_WRITE_WITH_IMM)
    int total_operations = num_writes + 1;
    int finished = 0;
    do {
        finished += wait_for_rdma_completion(res);
    } while (finished < total_operations);
    
    printf("All RDMA operations completed successfully (%d Writes + 1 Write_with_IMM)\n", num_writes);
    return 0;
}
...
int post_recv(struct rdma_resources *res) {
    ...
    printf("Receive request posted for RDMA_WRITE_WITH_IMM\n");
    return 0;
}
...
// 等待RDMA_WRITE_WITH_IMM的接收完成
int wait_for_write_with_imm_completion(struct rdma_resources *res) {
    struct ibv_wc wc;
    int n;
    
    printf("Waiting for RDMA_WRITE_WITH_IMM completion...\n");
    ...
    if (wc.status != IBV_WC_SUCCESS) {
        fprintf(stderr, "RDMA_WRITE_WITH_IMM completed with error: %s (status=%d, vendor_err=%d)\n", 
                ibv_wc_status_str(wc.status), wc.status, wc.vendor_err);
        return 0;
    }
    
    // 检查是否是RDMA_WRITE_WITH_IMM的接收完成
    if (wc.opcode == IBV_WC_RECV_RDMA_WITH_IMM) {
        uint32_t imm_data = ntohl(wc.imm_data);
        printf("Successfully received RDMA_WRITE_WITH_IMM!\n");
        printf("Immediate data: 0x%x (%u)\n", imm_data, imm_data);
        printf("Work Request ID: %ld\n", wc.wr_id);
        
        // 验证immediate data是否正确
        if (imm_data == 0x12345678) {
            printf("Immediate data verification PASSED!\n");
        } else {
            printf("Immediate data verification FAILED! Expected: 0x12345678, Got: 0x%x\n", imm_data);
        }
        return 1;
    } else {
        printf("Received completion for operation: %d (expected IBV_WC_RECV_RDMA_WITH_IMM)\n", wc.opcode);
        return 0;
    }
}
...
int main(int argc, char *argv[]) {
    ...
    if (is_sender) {
        ...
        printf("Waiting for RDMA Write operations and RDMA_WRITE_WITH_IMM...\n");
        
        // 等待RDMA_WRITE_WITH_IMM完成
        if (wait_for_write_with_imm_completion(&res)) {
            printf("Successfully received and verified RDMA_WRITE_WITH_IMM\n");
        } else {
            fprintf(stderr, "Failed to receive RDMA_WRITE_WITH_IMM\n");
            cleanup_rdma_resources(&res);
            return 1;
        }
    }
