...
int init_rdma_resources(struct rdma_resources *res, const char *dev_name, int ib_port, int gid_idx,
    ...
    // 加 16 防止溢出
    res->cq = ibv_create_cq(res->context, res->num_writes + MAX_RECV_WR + 16, NULL, NULL, 0);
    if (!res->cq) {
    ...
    struct ibv_qp_init_attr qp_init_attr = {
        ...
        .recv_cq = res->cq,
        .cap = {
            // 给发送队列增加 16 个空位的 Buffer
            .max_send_wr = res->is_sender ? res->num_writes + 16 : 1, 
            .max_recv_wr = MAX_RECV_WR,
            .max_send_sge = MAX_SGE,
            .max_recv_sge = MAX_SGE
        },
...
int connect_qp(struct rdma_resources *res) {
    ...
    attr.qp_state = IBV_QPS_RTS;
    // 14 大约对应 67 毫秒的超时，是比较推荐的默认值
    attr.timeout = 14; 
    // 发生丢包时，硬件最多自动重传 7 次
    attr.retry_cnt = 7; 
    // 接收端未准备好时的重试次数，7 通常代表无限重试
    attr.rnr_retry = 7;
...
int perform_rdma_write(struct rdma_resources *res, int num_writes) {
    ...
    // 一次性post整个链表 (Batch posting)
    struct ibv_send_wr *bad_wr = NULL;
    int ret = ibv_post_send(res->qp, &wrs[0], &bad_wr);
    if (ret) {
        // bad_wr 会指向那个引发失败的 WR
        int failed_id = bad_wr ? bad_wr->wr_id : -1;
        fprintf(stderr, "Failed to post send WR chain at ID #%d: %s\n", failed_id, strerror(ret));
        free(wrs);
        free(sges);
        return 1;
    }
