...
SingSinkLoggerSampling::SingSinkLoggerSampling(simtime_picosec period,
                                             EventList& eventlist):
    SinkLoggerSampling(period, eventlist, Logger::UEC_SINK, SingLogger::RATE)
{
    cout << "SingSinkLoggerSampling(p=" << timeAsSec(period) << " init \n";
}

void SingSinkLoggerSampling::doNextEvent(){
    ...
    //uint32_t deltaSnd = 0;                                                                                                                                                                                         
    double rate;
    //cout << "SingSinkLoggerSampling(p=" << timeAsSec(_period) << "), t=" << timeAsSec(now) << "\n";                                                                                                                
    for (uint64_t i = 0; i<_sinks.size(); i++){
        SingSink *sink = (SingSink*)_sinks[i];
...
string SingSinkLoggerSampling::event_to_str(RawLogEvent& event) {
    stringstream ss;
    ss << fixed << setprecision(9) << event._time;
    switch(event._type) {
    case Logger::UEC_SINK:
	assert(event._ev == SingLogger::RATE);
