...
bool SingPdcSes::_debug = false;
bool SingMsg::_output_completion_time = false;

SingMsg::SingMsg(SingPdcSes& pdc, msgid_t msg_id, mem_b size, bool debug): 
        _debug(debug),
        _pdc(pdc),
        _msg_id(msg_id),
...
pair<mem_b, bool> SingMsg::getNextSegment(SingDataPacket::seq_t seq_no, mem_b mss) {
    if (_debug) {
...
mem_b SingMsg::addRecvd(SingDataPacket::seq_t seq_no) {
    mem_b new_bytes = 0;
    uint32_t seq_cnt = _sent_pkt_notrecvd.count(seq_no);

    if (seq_cnt == 0) {
        // Duplicate pkt
        cout << "SingMsg::addRecvd " 
                << " " << _pdc._debug_tag
                << " msg id " << _msg_id
                << " seq no " << seq_no
...
mem_b SingMsg::addAck(SingDataPacket::seq_t ackno) {
...
optional<pair<SingDataPacket::seq_t,SingDataPacket::seq_t>> SingMsg::getSeq() {
...
void SingMsg::set_status(MsgStatus new_status) {
    ...
    if (_debug) {
        cout << timeAsUs(EventList::getTheEventList().now()) 
            << " SingMsg::set_status " << _pdc._debug_tag << " msg id " << _msg_id
...
bool SingMsg::status(MsgStatus status) {
    return _state >= status;
}

bool SingMsg::checkFinished() {
    return status(MsgStatus::Finished);
}

void SingMsg::activate() {
    mem_b new_bytes = _pdc.makeMsgEligible(this);

    _pdc.schedule_connection(new_bytes);
}

SingPdcSes::SingPdcSes(SingTransportConnection* connection,
                     EventList& eventlist,
                     mem_b mss, 
                     mem_b hdr_size,
                     string debug_tag): 
...
SingPdcSes::~SingPdcSes() {
    for (auto [_, msg]: _msgs) {
        delete msg;
    }
}

SingMsg::msgid_t SingPdcSes::get_next_msg_id()
{
...
SingMsg* SingPdcSes::enque(mem_b size, 
                      optional<simtime_picosec> scheduled_time, 
                      bool schedule_event) {
    SingMsg::msgid_t msg_id = get_next_msg_id();
    assert(!schedule_event or scheduled_time.has_value());
    SingMsg* msg = new SingMsg(*this, msg_id, size, _debug);
...
mem_b SingPdcSes::getNextPacket(SingDataPacket::seq_t seq_no) {
...
void SingPdcSes::notifyCtrlSeqno(SingDataPacket::seq_t seq_no) {
...
void SingPdcSes::addCumAck(SingDataPacket::seq_t cum_ack) {
    /*
    ...
    */
    ...
    map<SingDataPacket::seq_t, SingMsg*>::iterator seq_it;
...
void SingPdcSes::addSAck(SingDataPacket::seq_t ackno) {
    /*
    ...
    */
    ...
    SingMsg* cur_msg = _seq_to_msg.at(ackno);
...
void SingPdcSes::addRecvd(SingDataPacket::seq_t seq_no) {
    ...
    if (_seq_to_msg.find(seq_no) != _seq_to_msg.end()) {
        SingMsg* msg = _seq_to_msg.at(seq_no);
...
optional<pair<SingDataPacket::seq_t,SingDataPacket::seq_t>> 
SingPdcSes::getMsgSeq(SingMsg::msgid_t msg_id) {
    return _msgs.at(msg_id)->getSeq();
}

bool SingPdcSes::checkDoneSending() {
    bool cur_done_sending; 
    if (_cur_msg.has_value()) {
        cur_done_sending = _cur_msg.value()->status(SingMsg::MsgStatus::SentLast)
...
bool SingPdcSes::isTotallyFinished() {
    return checkFinished() && _total_pkt_bytes == _acked_pkt_bytes;
}

uint32_t SingPdcSes::getMsgCompleted() {
    return _msgs_complete.size();
}

mem_b SingPdcSes::eligiblePktSize() {
    return _eligible_pkt_bytes;
}

SingMsg* SingPdcSes::getMsg(SingMsg::msgid_t msg_id) {
    return _msgs.at(msg_id);
}

SingMsg::msgid_t SingPdcSes::getMsgId(SingDataPacket::seq_t seq_no) {
    return _seq_to_msg.at(seq_no)->msg_id();
}

mem_b SingPdcSes::updateScheduledMsgs(simtime_picosec now) {
...
mem_b SingPdcSes::makeMsgEligible(SingMsg* msg) {
...
void SingPdcSes::schedule_connection(mem_b new_bytes) {
    if (_debug) {
        cout << timeAsUs(eventlist().now())
