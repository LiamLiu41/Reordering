...
// UecNICPort: Responsible for physical packet transmission
// Manages busy state and triggers NIC scheduling after send completion
class UecNICPort : public EventSource {
public:
    UecNICPort(UecNIC& nic, linkspeed_bps linkspeed, EventList& eventlist);
...
public:
    ...
    void registerSrcForScheduling(UecSrc* src, simtime_picosec next_send_time);
    void unregisterSrc(UecSrc* src);
    void rescheduleSrc(UecSrc* src, simtime_picosec next_send_time);
...
private:
    // Port
    unique_ptr<UecNICPort> _port;
    
    // Data scheduling queue: (next_send_time, UecSrc*)
    std::multimap<simtime_picosec, UecSrc*> _data_schedule_queue;
    std::set<UecSrc*> _scheduled_srcs;  // Fast lookup
...
public:
    ...
    */
    ...
    // Path selection granularity control
    static mem_b _path_selection_granularity;  // in bytes, 0 means per-packet (default)

    // Intra-granularity path selection behavior
    enum IntraGranularityBehavior { KEEP_PATH, RANDOM_PATH };
    static IntraGranularityBehavior _intra_granularity_behavior;  // default: KEEP_PATH
...
public:
    mem_b getNextPacketSize();  // Made public for UecNIC scheduler
...
private:
    ...
    uint16_t getPathId(mem_b pkt_size);  // Get path ID with granularity control
    ...
    bool _path_initialized;  // Track if we've selected a path yet
...
class UecSink : public DataReceiver {
   public:
    ...
    UecAckPacket* sack(uint16_t path_id, UecBasePacket::seq_t seqno, UecBasePacket::seq_t acked_psn, bool ce, bool rtx_echo, bool path_sel_end = false);
