...
uint16_t UecMpOblivious::randomEntropy() {
...
UecMpBitmap::UecMpBitmap(uint16_t no_of_paths, bool debug, uint8_t ecn_penalty)
    : UecMultipath(debug),
      _no_of_paths(no_of_paths),
      _current_ev_index(0),
      _ev_skip_bitmap(),
...
uint16_t UecMpBitmap::randomEntropy() {
    // Purely random path selection
    uint16_t mask = _no_of_paths - 1;
    uint16_t entropy = rand() & mask;
    entropy |= _path_random ^ (_path_random & mask);  // set upper bits
...
uint16_t UecMpReps::randomEntropy() {
    // Purely random path selection
    return rand() % _no_of_paths;
}
...
uint16_t UecMpRepsLegacy::randomEntropy() {
    // Purely random path selection
    return rand() % _no_of_paths;
}


UecMpMixed::UecMpMixed(uint16_t no_of_paths, bool debug, uint8_t ecn_penalty)
    : UecMultipath(debug),
      _bitmap(UecMpBitmap(no_of_paths, debug, ecn_penalty)),
...
uint16_t UecMpMixed::randomEntropy() {
