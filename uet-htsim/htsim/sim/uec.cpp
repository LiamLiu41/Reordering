...
bool UecNIC::_debug = false;
bool UecNICPort::_debug = false;
bool UecSrc::_print_receiver_fct = true;
...
mem_b UecSrc::_path_selection_granularity = 0;  // 0 means per-packet (default behavior)
UecSrc::IntraGranularityBehavior UecSrc::_intra_granularity_behavior = UecSrc::KEEP_PATH;  // default: keep same path within granularity
/* End Path selection granularity */
...
UecNICPort::UecNICPort(UecNIC& nic, linkspeed_bps linkspeed, EventList& eventlist)
    : EventSource(eventlist, "UecNICPort"), _nic(nic), _linkspeed(linkspeed) {
    _busy = false;
    _send_end_time = 0;
    _nodename = "UecNICPort_" + to_string(nic._src_id);
}

void UecNICPort::sendDataPacket(mem_b pkt_size) {
...
UecNIC::UecNIC(id_t src_num, EventList& eventList, linkspeed_bps linkspeed, uint32_t ports)
    ...
    _port = make_unique<UecNICPort>(*this, linkspeed, eventList);
...
void UecNIC::registerSrcForScheduling(UecSrc* src, simtime_picosec next_send_time) {
    // Avoid duplicate registration
    if (_scheduled_srcs.find(src) != _scheduled_srcs.end()) {
        unregisterSrc(src);
    }
    
    _data_schedule_queue.insert({next_send_time, src});
    _scheduled_srcs.insert(src);
    
    // If port is idle, check if we need to trigger scheduling
    if (!_port->isBusy()) {
        auto earliest = _data_schedule_queue.begin();
        if (earliest->second == src) {
            // This is the earliest
            if (earliest->first <= eventlist().now()) {
                // Time has arrived, trigger immediately
                scheduleNext();
            } else {
                // Not yet time, schedule to that time
...
void UecNIC::unregisterSrc(UecSrc* src) {
    if (_scheduled_srcs.find(src) == _scheduled_srcs.end()) {
        return;  // Not in queue
    }
    
    // Remove from multimap (may have multiple entries with same time)
    ...
    _scheduled_srcs.erase(src);
}

void UecNIC::rescheduleSrc(UecSrc* src, simtime_picosec next_send_time) {
    registerSrcForScheduling(src, next_send_time);
}
...
void UecNIC::scheduleNext() {
    ...
    while (!_data_schedule_queue.empty()) {
        ...
        if (src->hasPacketsToSend() && src->canSendNewPacket()) {
            const Route* route = src->getPortRoute(0);
            mem_b sent_bytes = src->sendPacketFromNIC(*route);
            
            if (sent_bytes > 0) {
                // Send successful, notify port
                _port->sendDataPacket(sent_bytes);
                
                // Calculate next send time and re-register
                mem_b next_pkt_size = src->getNextPacketSize();
                simtime_picosec next_time = src->computeNextSendTime(next_pkt_size);
                registerSrcForScheduling(src, next_time);
        ...
        if (next_pkt_size == 0) {
            ...
            next_time = src->computeNextSendTime(next_pkt_size);
        }
...
uint32_t UecNIC::getMaxConcurrentFlows() const {
    return _max_concurrent_flows;
}

simtime_picosec UecNIC::getMaxConcurrentFlowsTime() const {
    return _max_concurrent_flows_time;
}

uint32_t UecNIC::getActiveFlowCount() const {
    return _active_flow_count;
}

double UecNIC::getAverageConcurrentFlows(simtime_picosec simulation_end_time) const {
...
UecSrc::UecSrc(TrafficLogger* trafficLogger, 
    ...
    // Initialize path selection granularity tracking
    _bytes_sent_since_last_path_update = 0;
    _current_pathid = 0;  // Will be set on first call to getPathId()
    _path_initialized = false;  // Path not yet selected
}
...
void UecSrc::processAck(const UecAckPacket& pkt) {
    ...
    if (_mp->getType() == UecMultipath::REPS) {
        // Using UecMpReps - only process if this is a path_sel_end ACK
...
uint16_t UecSrc::getPathId(mem_b pkt_size) {
    if (_path_selection_granularity == 0) {
        // Per-packet path selection (default behavior)
        return _mp->nextEntropy(_highest_sent, (uint64_t)_cwnd/_mss);
    } else {
        // Granularity-based path selection
        
        // First time: initialize the path
        if (!_path_initialized) {
            _current_pathid = _mp->nextEntropy(_highest_sent, (uint64_t)_cwnd/_mss);
            _bytes_sent_since_last_path_update = pkt_size;
            _path_initialized = true;
            
            if (_debug_src || _flow.flow_id() == _debug_flowid) {
                cout << timeAsUs(eventlist().now()) << " flowid " << _flow.flow_id() 
                     << " initializing path to " << _current_pathid << endl;
            }
            
            return _current_pathid;
        }
        
        // Accumulate bytes sent
        _bytes_sent_since_last_path_update += pkt_size;
        
        // Check if we need to update the path (granularity boundary reached)
        if (_bytes_sent_since_last_path_update >= _path_selection_granularity) {
            ...
            if (_debug_src || _flow.flow_id() == _debug_flowid) {
                cout << timeAsUs(eventlist().now()) << " flowid " << _flow.flow_id() 
                     << " updating path to " << _current_pathid 
                     << " after " << _path_selection_granularity << " bytes" << endl;
            }
            
            return _current_pathid;
        }
        
        // Within granularity: behavior depends on _intra_granularity_behavior
        if (_intra_granularity_behavior == RANDOM_PATH) {
            // Purely random path for each packet within granularity
            return _mp->randomEntropy();
        } else {
            // KEEP_PATH: keep using the same path within granularity (default)
        return _current_pathid;
        }
    }
}
...
void UecSrc::runSleek(uint32_t ooo, UecBasePacket::seq_t cum_ack) {
    ...
    // In new architecture, queueForRtx() already reschedules with NIC
}
...
void UecSrc::startConnection() {
    ...
    _rtx_backlog = 0;
    _max_unack_window = 0;  // Initialize (0 means no limit)

    // Register with NIC scheduler for immediate sending
    _nic.registerSrcForScheduling(this, eventlist().now());
    
    if (_debug_src) {
        cout << _flow.str() << " registered with NIC scheduler at "
             << timeAsUs(eventlist().now()) << endl;
    }
}
...
bool UecSrc::hasPacketsToSend() const {
    return (_backlog > 0 || !_rtx_queue.empty());
}

bool UecSrc::canSendNewPacket() const {
    // Always can send retransmission packets
    if (!_rtx_queue.empty()) {
        return true;
    }
    
    // Check max_unack_window limit (for future extension)
    if (_max_unack_window > 0 && _in_flight >= _max_unack_window) {
        return false;
    }
    
    // Other future constraints can be added here
    
    return true;
}

simtime_picosec UecSrc::computeNextSendTime(mem_b pkt_size) const {
    if (pkt_size == 0) {
        // No packets to send, wait one base_rtt
        return eventlist().now() + _base_rtt;
    }
    
    if (_cwnd == 0 || _base_rtt == 0) {
        // Defensive check
        return eventlist().now() + _base_rtt;
    }
    
    // rate = cwnd / base_rtt (bytes per picosec)
    double rate = (double)_cwnd / (double)_base_rtt;
    
    // interval = pkt_size / rate
    simtime_picosec interval = (simtime_picosec)((double)pkt_size / rate);
    
    // Minimum interval to prevent division issues
    if (interval < 100) {  // Minimum 100 picoseconds
        interval = 100;
    }
    
    return eventlist().now() + interval;
}

mem_b UecSrc::sendPacketFromNIC(const Route& route) {
    // Called by NIC to actually send a packet
    mem_b sent_bytes = 0;
    
    if (!_rtx_queue.empty()) {
        sent_bytes = sendRtxPacket(route);
    } else if (_backlog > 0) {
        sent_bytes = sendNewPacket(route);
    }
    
    // Note: No need to call rescheduleSrc here, NIC's scheduleNext() handles it
...
void UecSrc::sendIfPermitted() {
    // DEPRECATED: This function is no longer used in the new scheduling architecture
    // NIC scheduler handles all sending decisions
    // Kept as stub for potential legacy code paths
    
    if (_debug_src)
        cout << "WARNING: sendIfPermitted() called, but deprecated in new scheduler" << endl;
}
...
mem_b UecSrc::sendNewPacket(const Route& route) {
    ...
    // Check if this packet will be the last in current path selection unit
    bool is_path_sel_end = false;
    if (_path_selection_granularity > 0) {
        // If after sending this packet we'll reach or exceed the granularity, 
        // this is the last packet in the current path selection unit
        if (_bytes_sent_since_last_path_update + full_pkt_size >= _path_selection_granularity) {
            is_path_sel_end = true;
        }
    }

    uint16_t ev = getPathId(full_pkt_size);
    p->set_pathid(ev);
    p->set_path_sel_end(is_path_sel_end);
    ...
    if (_flow.flow_id() == _debug_flowid)
    {
        cout << timeAsUs(eventlist().now()) << " flowid " << _flow.flow_id() <<" sending pkt " << _highest_sent
             ...
             << " ar " << p->ar()
             << " path_sel_end " << (is_path_sel_end ? "YES" : "NO")
             << endl;
    }
    p->sendOn();
    _highest_sent++;
...
void UecSrc::sendProbe() {
    ...
    p->set_dst(_dstaddr);
    uint16_t ev = getPathId(_hdr_size);
    p->set_pathid(ev);
    // p->sendOn();
...
void UecSrc::queueForRtx(UecBasePacket::seq_t seqno, mem_b pkt_size) {
    ...
    // Register with NIC scheduler for immediate sending
    _nic.rescheduleSrc(this, eventlist().now());
}

void UecSrc::timeToSend(const Route& route) {
    // DEPRECATED: This function is no longer used in the new scheduling architecture
    // NIC scheduler directly calls sendPacketFromNIC()
    // Kept as stub for potential legacy code paths
    
    if (_debug_src)
        cout << "WARNING: timeToSend() called, but deprecated in new scheduler" << endl;
}
...
void UecSrc::rtxTimerExpired() {
    ...
    // In new architecture, queueForRtx() automatically reschedules with NIC
    // No need to manually request sending
    
    // For receiver-based CC, still send RTS if needed
    if (_receiver_based_cc && (!_rtx_queue.empty() || _credit <= 0)) {
        if (_debug_src)
            cout << "sendRTS (RTO expired)"
                 << " flow " << _flow.str() << endl;
        sendRTS();
    }
}
...
void UecSink::processData(UecDataPacket& pkt) {
    ...
    if (pkt.packet_type() == UecBasePacket::DATA_PROBE){
        UecAckPacket* ack_packet =
            sack(pkt.path_id(), sackBitmapBase(pkt.epsn()), pkt.epsn(), (bool)(pkt.flags() & ECN_CE), pkt.retransmitted(), false);
    ...
    if (pkt.epsn() < _expected_epsn || _epsn_rx_bitmap[pkt.epsn()]) {
        ...
        UecAckPacket* ack_packet =
            sack(pkt.path_id(), ecn ? pkt.epsn() : sackBitmapBase(pkt.epsn()), pkt.epsn(), ecn, pkt.retransmitted(), false);
        _nic.sendControlPacket(ack_packet, NULL, this);
    ...
    // Check if receiver has completed receiving the flow
    if (!_receiver_done && _received_bytes >= _src->flowsize()) {
        _receiver_done = true;
        
        if (UecSrc::_print_receiver_fct) {
            ...
            cout << "receiver_fct: Flow " << _src->_name << " flowId " << _src->flowId() << " " << _nodename 
    ...
    // If packet is marked as path_sel_end, force an immediate ACK
    if (pkt.path_sel_end()) {
        force_ack = true;
        if (_src->flow()->flow_id() == UecSrc::_debug_flowid) {
            cout << timeAsUs(_src->eventlist().now()) << " flowid " << _src->flow()->flow_id()
                 << " recv path_sel_end pkt " << pkt.epsn() << " forcing ACK" << endl;
        }
    }
    ...
    if (ecn || shouldSack() || force_ack) {
        UecAckPacket* ack_packet =
            sack(pkt.path_id(), (ecn || pkt.ar()) ? pkt.epsn() : sackBitmapBase(pkt.epsn()), pkt.epsn(), ecn, pkt.retransmitted(), pkt.path_sel_end());
        ...
        if (_src->flow()->flow_id() == UecSrc::_debug_flowid)
        {
            cout << timeAsUs(_src->eventlist().now()) << " flowid " << _src->flow()->flow_id()
                 ...
                 << " ooo_count " << _out_of_order_count
                 << " recvd_bytes " << _recvd_bytes << " flow " << _src->flow()->str() 
                 << " ecn " << ecn << " shouldSack " << shouldSack() << " forceack " << force_ack
...
void UecSink::processTrimmed(const UecDataPacket& pkt) {
    ...
    if (pkt.epsn() < _expected_epsn || _epsn_rx_bitmap[pkt.epsn()]) {
        ...
        UecAckPacket* ack_packet = sack(pkt.path_id(), sackBitmapBase(pkt.epsn()), pkt.epsn(), false, pkt.retransmitted(), false);
        //ack_packet->sendOn();
        _nic.sendControlPacket(ack_packet, NULL, this);
        return;
    }
...
void UecSink::processRts(const UecRtsPacket& pkt) {
    ...
    if (pkt.epsn() < _expected_epsn || _epsn_rx_bitmap[pkt.epsn()]) {
        ...
        // this code is different from the proposed hardware implementation, as it keeps track of
        // the ACK state of OOO packets.
        UecAckPacket* ack_packet = sack(pkt.path_id(), sackBitmapBase(pkt.epsn()), pkt.epsn(), ecn, pkt.retransmitted(), false);
    ...
    UecAckPacket* ack_packet =
        sack(pkt.path_id(), (ecn || pkt.ar()) ? pkt.epsn() : sackBitmapBase(pkt.epsn()), pkt.epsn(), ecn, pkt.retransmitted(), false);
    ack_packet->set_is_rts(true);
    if (_src->debug())
        cout << " UecSink " << _nodename << " src " << _src->nodename()
...
UecAckPacket* UecSink::sack(uint16_t path_id, UecBasePacket::seq_t seqno, UecBasePacket::seq_t acked_psn, bool ce, bool rtx_echo, bool path_sel_end) {
