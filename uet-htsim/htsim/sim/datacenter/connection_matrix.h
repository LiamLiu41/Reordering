...
struct connection{
    ...
    int priority;
    simtime_picosec ideal_time;  // ideal completion time: size/bw + rtt
    string type;  // flow type for classification (e.g., "query", "background", etc.)
    optional<CcAlgoId> cc_algo;    // optional per-flow CC algorithm override
    optional<string> cc_profile;   // optional per-flow CC profile-id override
    uint32_t cm_line;              // source line in .cm (0 if unavailable)
};
