...
public:
    ...
    */
    ...
    void set_failed_link_ratio(double ratio) {
...
public:
    enum switch_model_t { FATTREE_SWITCH = 0, SING_SWITCH = 1 };
    switch_model_t _switch_model = FATTREE_SWITCH;
    uint64_t _sing_switch_buffer = 64ULL * 1024 * 1024;

    uint8_t _pfc_mask = 0;
    double _pfc_alpha = 0.125;
    uint64_t _pfc_resume_offset = 0;
    uint64_t _pfc_xoff = 0;
    uint64_t _pfc_xon = 0;
    uint64_t _pfc_headroom = 0;
private:
...
public:
    FatTreeTopology(const FatTreeTopologyCfg* cfg,
                    ...
                    const char* interest_file = nullptr);
...
private:
    ...
    // Interest queue filtering
    set<string> _interest_base_queues;
    map<string, uint16_t> _interest_prio_masks;
    void load_interest_queues(const string& filename);
    bool is_queue_of_interest(const string& queue_name) const;
    vector<int> get_sing_interest_prios(const string& base_name, int num_prios) const;
    void attach_sing_queue_loggers(SingSwitch* sw, int out_port, const string& base_name);
};
