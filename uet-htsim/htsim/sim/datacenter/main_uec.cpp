...
simtime_picosec calculate_rtt_between_hosts(FatTreeTopologyCfg* t_cfg, uint32_t src, uint32_t dst, linkspeed_bps host_linkspeed) {
    ...
    simtime_picosec ack_serialization = (UecBasePacket::get_ack_size() * 8 * 1e12) / host_linkspeed;
...
//            cout << "fair_decrease disabled" << endl;
            ...
            UecSrc::_print_receiver_fct = false;
    ...
    */
    ...
    // Set path selection granularity
    UecSrc::_path_selection_granularity = path_selection_granularity;
    if (path_selection_granularity == 0) {
    ...
    for (size_t c = 0; c < all_conns->size(); c++){
        ...
        if (!conn_reuse 
            ...
            if (load_balancing_algo == BITMAP){
                mp = make_unique<UecMpBitmap>(path_entropy_size, UecSrc::_debug, ecn_penalty);
                ...
                mp = make_unique<UecMpMixed>(path_entropy_size, UecSrc::_debug, ecn_penalty);
            } else {
