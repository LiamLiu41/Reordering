...
std::ostream &operator<<(std::ostream &os, FatTreeTopologyCfg const &m) { 
    os << "FatTreeTopologyCfg"
       << " NCORE=" << m.NCORE 
        ...
        << " pfc_mask=" << (uint32_t)m._pfc_mask
        << " pfc_alpha=" << m._pfc_alpha
        << " pfc_resume_offset=" << m._pfc_resume_offset
        << " pfc_xoff=" << m._pfc_xoff
        << " pfc_xon=" << m._pfc_xon
        << " pfc_headroom=" << m._pfc_headroom
...
FatTreeTopology::FatTreeTopology(const FatTreeTopologyCfg* cfg,
    ...
    // Try to load interest queues from file if specified
    if (interest_file != nullptr && interest_file[0] != '\0') {
        load_interest_queues(interest_file);
    } else {
        cout << "No interest queue file specified. Queue logging disabled." << endl;
    }
    ...
    //create switches
    bool use_sing_switch = (_cfg->_switch_model == FatTreeTopologyCfg::SING_SWITCH);
    for (uint32_t j=0;j<_cfg->NTOR;j++){
        simtime_picosec switch_latency = (_cfg->_switch_latencies[TOR_TIER] > 0) ? _cfg->_switch_latencies[TOR_TIER] : _cfg->_switch_latency;
        if (use_sing_switch)
            switches_lp[j] = new SingSwitch(*_eventlist, "Switch_LowerPod_"+ntoa(j),FatTreeSwitch::TOR,j,switch_latency,this,_cfg->_sing_switch_buffer);
        else
    ...
    for (uint32_t j=0;j<_cfg->NAGG;j++){
        ...
        if (use_sing_switch)
            switches_up[j] = new SingSwitch(*_eventlist, "Switch_UpperPod_"+ntoa(j),FatTreeSwitch::AGG,j,switch_latency,this,_cfg->_sing_switch_buffer);
        else
    ...
    for (uint32_t j=0;j<_cfg->NCORE;j++){
        ...
        if (use_sing_switch)
            switches_c[j] = new SingSwitch(*_eventlist, "Switch_Core_"+ntoa(j),FatTreeSwitch::CORE,j,switch_latency,this,_cfg->_sing_switch_buffer);
        else
    ...
    for (uint32_t tor = 0; tor < _cfg->NTOR; tor++) {
        ...
        for (uint32_t l = 0; l < link_bundles; l++) {
            ...
            for (uint32_t b = 0; b < _cfg->_bundlesize[TOR_TIER]; b++) {
                // Downlink
                string downlink_name = "LS" + ntoa(tor) + "->DST" +ntoa(srv) + "(" + ntoa(b) + ")";
                if (!use_sing_switch && _logger_factory && is_queue_of_interest(downlink_name)) {
                ...
                if (!use_sing_switch) {
                    ...
                    queues_nlp_ns[tor][srv][b]->setName(downlink_name);
                    if (_logfile) _logfile->writeName(*(queues_nlp_ns[tor][srv][b]));
                    assert(switches_lp[tor]->addPort(queues_nlp_ns[tor][srv][b]) < 96);
                } else {
                    queues_nlp_ns[tor][srv][b] = NULL;
                }

                // Uplink (server to TOR - always created, this is the host-side queue)
                string uplink_name = "SRC" + ntoa(srv) + "->LS" +ntoa(tor) + "(" + ntoa(b) + ")";
                if (_logger_factory && is_queue_of_interest(uplink_name)) {
                    queueLogger = _logger_factory->createQueueLogger();
                } else {
                    queueLogger = NULL;
                }
                queues_ns_nlp[srv][tor][b] = alloc_src_queue(queueLogger);   
                queues_ns_nlp[srv][tor][b]->setName(uplink_name);
                if (_logfile) _logfile->writeName(*(queues_ns_nlp[srv][tor][b]));

                queues_ns_nlp[srv][tor][b]->setRemoteEndpoint(switches_lp[tor]);

                if (!use_sing_switch && (cfg->_qt==LOSSLESS_INPUT || cfg->_qt == LOSSLESS_INPUT_ECN)){
                ...
                if (use_sing_switch) {
                    SingSwitch* sw = static_cast<SingSwitch*>(switches_lp[tor]);
                    int port_idx = sw->addOutputPort(
                        _cfg->_downlink_speeds[TOR_TIER],
                        pipes_nlp_ns[tor][srv][b],
                        pipes_ns_nlp[srv][tor][b],
                        queues_ns_nlp[srv][tor][b]);
                    attach_sing_queue_loggers(sw, port_idx, downlink_name);
                }
            
                if (_ff){
                    if (queues_nlp_ns[tor][srv][b])
    ...
    for (uint32_t tor = 0; tor < _cfg->NTOR; tor++) {
        ...
        for (uint32_t agg=agg_min; agg<=agg_max; agg++){
            for (uint32_t b = 0; b < _cfg->_bundlesize[AGG_TIER]; b++) {
                // Downlink
                string downlink_name = "US" + ntoa(agg) + "->LS" + ntoa(tor) + "(" + ntoa(b) + ")";
                string uplink_name = "LS" + ntoa(tor) + "->US" + ntoa(agg) + "(" + ntoa(b) + ")";
                if (!use_sing_switch && _logger_factory && is_queue_of_interest(downlink_name)) {
                    queueLogger = _logger_factory->createQueueLogger();
                } else {
                    queueLogger = NULL;
                ...
                // Downlink pipe (AGG->TOR) - always created
                ...
                // Uplink pipe (TOR->AGG) - always created
                ...
                if (use_sing_switch) {
                    ...
                    bool failed_tier2_link = (_cfg->_tiers == 2 &&
                                              _cfg->_num_failed_links > 0 &&
                                              agg == 0 && tor == 0);
                    linkspeed_bps speed_up_to_lp = _cfg->_downlink_speeds[AGG_TIER];
                    linkspeed_bps speed_lp_to_up = _cfg->_downlink_speeds[AGG_TIER];
                    if (failed_tier2_link) {
                        speed_up_to_lp = (linkspeed_bps)(speed_up_to_lp * _cfg->_failed_link_ratio);
                        speed_lp_to_up = (linkspeed_bps)(speed_lp_to_up * _cfg->_failed_link_ratio);
                    }
                    SingSwitch* sw_up = static_cast<SingSwitch*>(switches_up[agg]);
                    int up_port_idx = sw_up->addOutputPort(
                        speed_up_to_lp,
                        pipes_nup_nlp[agg][tor][b],
                        pipes_nlp_nup[tor][agg][b],
                        switches_lp[tor]);
                    attach_sing_queue_loggers(sw_up, up_port_idx, downlink_name);

                    SingSwitch* sw_lp = static_cast<SingSwitch*>(switches_lp[tor]);
                    int lp_port_idx = sw_lp->addOutputPort(
                        speed_lp_to_up,
                        pipes_nlp_nup[tor][agg][b],
                        pipes_nup_nlp[agg][tor][b],
                        switches_up[agg]);
                    attach_sing_queue_loggers(sw_lp, lp_port_idx, uplink_name);
                    if (failed_tier2_link) {
                        cout << "Failure: US" + ntoa(agg) + "->LS_" + ntoa(tor) + "(" + ntoa(b) + ") linkspeed set to "
                             << speedAsGbps(speed_up_to_lp) << endl;
                        cout << "Failure: LS" + ntoa(tor) + "->US" + ntoa(agg) + "(" + ntoa(b) + ") linkspeed set to "
                             << speedAsGbps(speed_lp_to_up) << endl;
                    }
                } else {
                    // Downlink queue (AGG->TOR)
                    if (_cfg->_tiers == 2 && _cfg->_num_failed_links > 0 && agg == 0 && tor == 0){
                    ...
                    queues_nup_nlp[agg][tor][b]->setName(downlink_name);
                    if (_logfile) _logfile->writeName(*(queues_nup_nlp[agg][tor][b]));

                    // Uplink queue (TOR->AGG)
                    if (_logger_factory && is_queue_of_interest(uplink_name)) {
                        queueLogger = _logger_factory->createQueueLogger();
                    } else {
                        queueLogger = NULL;
        ...
        for (uint32_t agg = 0; agg < _cfg->NAGG; agg++) {
            ...
            for (uint32_t l = 0; l < _cfg->_radix_up[AGG_TIER]/_cfg->_bundlesize[CORE_TIER]; l++) {
                ...
                for (uint32_t b = 0; b < _cfg->_bundlesize[CORE_TIER]; b++) {
                    ...
                    // Uplink (Agg to Core)
                    string uplink_name = "US" + ntoa(agg) + "->CS" + ntoa(core) + "(" + ntoa(b) + ")";
                    string downlink_name = "CS" + ntoa(core) + "->US" + ntoa(agg) + "(" + ntoa(b) + ")";
                    if (!use_sing_switch && _logger_factory && is_queue_of_interest(uplink_name)) {
                        queueLogger = _logger_factory->createQueueLogger();
                    } else {
                        queueLogger = NULL;
                    }
                    ...
                    // Uplink pipe (AGG->CORE) - always created
                    ...
                    // Downlink pipe (CORE->AGG) - always created
                    ...
                    if (use_sing_switch) {
                        queues_nup_nc[agg][core][b] = NULL;
                        queues_nc_nup[core][agg][b] = NULL;
                        SingSwitch* sw_up = static_cast<SingSwitch*>(switches_up[agg]);
                        int up_port_idx = sw_up->addOutputPort(
                            _cfg->_downlink_speeds[CORE_TIER],
                            pipes_nup_nc[agg][core][b],
                            pipes_nc_nup[core][agg][b],
                            switches_c[core]);
                        attach_sing_queue_loggers(sw_up, up_port_idx, uplink_name);

                        SingSwitch* sw_core = static_cast<SingSwitch*>(switches_c[core]);
                        int core_port_idx = sw_core->addOutputPort(
                            _cfg->_downlink_speeds[CORE_TIER],
                            pipes_nc_nup[core][agg][b],
                            pipes_nup_nc[agg][core][b],
                            switches_up[agg]);
                        attach_sing_queue_loggers(sw_core, core_port_idx, downlink_name);
                    } else {
                        ...
                        queues_nup_nc[agg][core][b]->setName(uplink_name);
                        if (_logfile) _logfile->writeName(*(queues_nup_nc[agg][core][b]));

                        if (_logger_factory && is_queue_of_interest(downlink_name)) {
                            queueLogger = _logger_factory->createQueueLogger();
                        } else {
                            queueLogger = NULL;
                        }
                        ...
                        queues_nc_nup[core][agg][b]->setName(downlink_name);
                    ...
                    if (_ff){
                        if (queues_nup_nc[agg][core][b])
                            _ff->add_queue(queues_nup_nc[agg][core][b]);
                        if (queues_nc_nup[core][agg][b])
    ...
    if (use_sing_switch && _cfg->_enable_ecn) {
        auto configure_ecn = [&](Switch* sw) {
            SingSwitch* ss = static_cast<SingSwitch*>(sw);
            for (int p = 0; p < ss->numOutputPorts(); p++) {
                for (int q = 0; q < ss->bufferManager().numPrios(); q++) {
                    ss->bufferManager().setEcnThreshold(p, q,
                        _cfg->_ecn_low, _cfg->_ecn_high);
                }
            }
        };
        for (uint32_t j = 0; j < _cfg->NTOR; j++) configure_ecn(switches_lp[j]);
        for (uint32_t j = 0; j < _cfg->NAGG; j++) configure_ecn(switches_up[j]);
        for (uint32_t j = 0; j < _cfg->NCORE; j++) configure_ecn(switches_c[j]);
    }

    if (use_sing_switch) {
        auto configure_pfc = [&](Switch* sw) {
            SingSwitch* ss = static_cast<SingSwitch*>(sw);
            ss->pfcEngine().setPfcEnabledMask(_cfg->_pfc_mask);
            for (int p = 0; p < ss->numOutputPorts(); p++) {
                for (int q = 0; q < ss->bufferManager().numPrios(); q++) {
                    if (ss->pfcEngine().isPfcEnabled(q)) {
                        // Backward compatibility: static xoff/xon knobs remain configurable
                        // even though the active shared-buffer model uses dynamic thresholds.
                        ss->pfcEngine().setThresholds(p, q, _cfg->_pfc_xoff, _cfg->_pfc_xon);
                    }
                }
            }
            if (!ss->configureDynamicPfc(_cfg->_pfc_alpha, _cfg->_pfc_resume_offset,
                                         _cfg->_pfc_headroom)) {
                cerr << "FATAL: failed to configure dynamic PFC model for "
                     << ss->nodename() << endl;
                abort();
            }
        };
        for (uint32_t j = 0; j < _cfg->NTOR; j++) configure_pfc(switches_lp[j]);
        for (uint32_t j = 0; j < _cfg->NAGG; j++) configure_pfc(switches_up[j]);
        for (uint32_t j = 0; j < _cfg->NCORE; j++) configure_pfc(switches_c[j]);
    }
}
    ...
    case COMPOSITE:
        {
            ...
            if (_cfg->_enable_ecn){
                if (!tor || dir == UPLINK || _cfg->_enable_ecn_on_tor_downlink) {
                        ...
                        // Keep ECN thresholds uniform across all links, including failed links.
...
vector<const Route*>* FatTreeTopology::get_bidir_paths(uint32_t src, uint32_t dest, bool reverse){
    if (_cfg->_switch_model == FatTreeTopologyCfg::SING_SWITCH) {
        cerr << "FATAL: get_bidir_paths() is not supported with SingSwitch (src="
             << src << ", dst=" << dest << "). "
             << "SingSwitch uses hop-by-hop FIB routing (ECMP_FIB style), not explicit end-to-end routes."
             << endl;
        abort();
    }
...
void FatTreeTopology::add_switch_loggers(Logfile& log, simtime_picosec sample_period) {
    if (_cfg->_switch_model == FatTreeTopologyCfg::SING_SWITCH) {
        cerr << "Warning: -log switch is not supported for SingSwitch yet; "
             << "skip switch-level aggregate loggers." << endl;
        return;
    }
...
void FatTreeTopology::load_interest_queues(const string& filename) {
    _interest_base_queues.clear();
    _interest_prio_masks.clear();

    ifstream infile(filename);
    if (!infile.is_open()) {
        cerr << "Warning: Interest queue file '" << filename
             << "' not found. Queue logging disabled." << endl;
        return;
    }
    ...
    while (getline(infile, line)) {
        ...
        // Parse only the first token of each line.
        ...
        size_t pos = token.rfind(":P");
        if (pos != string::npos) {
            string base_name = token.substr(0, pos);
            string prio_str = token.substr(pos + 2);

            char* end = nullptr;
            long prio = std::strtol(prio_str.c_str(), &end, 10);
            bool valid = !base_name.empty() &&
                         !prio_str.empty() &&
                         end != nullptr &&
                         *end == '\0' &&
                         prio >= 0 &&
                         prio < Packet::NUM_PRIOS;
            if (!valid) {
                cerr << "Warning: invalid interest entry '" << token
                     << "' at " << filename << ":" << line_no
                     << ". Expected <base> or <base>:P<n> with 0 <= n < "
                     << Packet::NUM_PRIOS << ". Ignored." << endl;
                continue;
            }

            _interest_prio_masks[base_name] |= (uint16_t)(1U << prio);
            valid_entries++;
            continue;
        }

        // ':' is reserved for SingSwitch per-priority syntax.
        if (token.find(':') != string::npos) {
            cerr << "Warning: invalid interest entry '" << token
                 << "' at " << filename << ":" << line_no
                 << ". Expected <base> or <base>:P<n>. Ignored." << endl;
            continue;
        }

        _interest_base_queues.insert(token);
        valid_entries++;
    }

    infile.close();
    cout << "Loaded " << valid_entries << " interest entries from '"
         << filename << "'" << endl;
    if (valid_entries == 0) {
        cout << "Warning: Interest queue file is empty or has no valid entries. "
             << "Queue logging disabled." << endl;
    }
}

bool FatTreeTopology::is_queue_of_interest(const string& queue_name) const {
    return _interest_base_queues.find(queue_name) != _interest_base_queues.end();
}

vector<int> FatTreeTopology::get_sing_interest_prios(const string& base_name, int num_prios) const {
    set<int> prios;
    if (num_prios <= 0) {
        return vector<int>();
    }

    if (_interest_base_queues.find(base_name) != _interest_base_queues.end() &&
        1 < num_prios) {
        prios.insert(1);
    }

    auto it = _interest_prio_masks.find(base_name);
    if (it != _interest_prio_masks.end()) {
        uint16_t mask = it->second;
        for (int prio = 0; prio < num_prios && prio < Packet::NUM_PRIOS; prio++) {
            if ((mask >> prio) & 1U) {
                prios.insert(prio);
            }
        }
    ...
    return vector<int>(prios.begin(), prios.end());
}

void FatTreeTopology::attach_sing_queue_loggers(SingSwitch* sw, int out_port,
                                                const string& base_name) {
    if (!_logger_factory || !sw) {
        return;
    }

    vector<int> prios = get_sing_interest_prios(base_name, sw->bufferManager().numPrios());
    for (int prio : prios) {
        QueueLogger* queue_logger = _logger_factory->createQueueLogger();
        string queue_name = base_name + ":P" + ntoa(prio);
        sw->bindQueueLogger(out_port, prio, queue_logger, queue_name);
        if (_logfile) {
            _logfile->writeName(sw->queue(out_port, prio));
        }
    }
}
