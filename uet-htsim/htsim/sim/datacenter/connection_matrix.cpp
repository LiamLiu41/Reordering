    ...
    */
    ...
    do {
        ...
        if (dstix != string::npos) {
            ...
            c->priority = 2000000;
            c->start = NO_START;
            c->type = "";  // default: empty type
            c->cc_algo = std::nullopt;
            c->cc_profile = std::nullopt;
            c->cm_line = static_cast<uint32_t>(linecount);
            for (size_t i = 1; i < tokens.size(); i++) {
                if (tokens[i] == "start") {
                    i++;
                    double start = stof(tokens[i]);
                    c->start = start; // start is in microseconds
                    ...
                    i++;
                    c->type = tokens[i];
                } else if (tokens[i] == "cc_algo") {
                    i++;
                    if (i >= tokens.size()) {
                        cerr << "Error: cc_algo requires a value at line " << linecount << endl;
                        exit(1);
                    }
                    CcAlgoId parsed_algo;
                    if (!CcProfileFlowSelectionResolver::parseAlgo(tokens[i], &parsed_algo)) {
                        cerr << "Error: unsupported cc_algo '" << tokens[i]
                             << "' at line " << linecount
                             << ", expected one of nscc|dctcp|constant|swift|barre" << endl;
                        exit(1);
                    }
                    c->cc_algo = parsed_algo;
                } else if (tokens[i] == "cc_profile") {
                    i++;
                    if (i >= tokens.size()) {
                        cerr << "Error: cc_profile requires a value at line " << linecount << endl;
                        exit(1);
                    }
                    c->cc_profile = tokens[i];
